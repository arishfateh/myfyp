...
export class AttractionAddComponent implements OnInit {
  ...
  public type: Array<string> = ["adventurous", "aesthetic", "All"]; // Type of the Attraction
  constructor(
  ...
  // To reset all the fields and empty the data that was placed in them
  ...
  // While submitting all the data for attaction point, this function is called to set the data from input form
  onSubmit(value) {
  ...
  // To Remove the image of an attraction Point that has been selected
  removeImage(ims) {
    this.imgsrcs = this.imgsrcs.filter((obj) => obj != ims);

    for (const key in this.attraction.imgsrcs) {
      if (
        JSON.stringify(this.attraction.imgsrcs[key]).toLowerCase() ===
