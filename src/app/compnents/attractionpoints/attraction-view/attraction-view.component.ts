...
export class AttractionViewComponent implements OnInit {
  ...
  constructor(public Service: AttractionPointService, private snackBar: MatSnackBar,
     private sanitizer: DomSanitizer, public foral: ForallService) { }
      ...
      if (this.attractionList.length > 0 && this.check == 0) {
        ...
        for (var i = 0; i < this.attractionList.length; i++) {
          for (let key in this.attractionList[i].imgsrcs) {
            this.attractionList[i].imgsrcs[key] = this.sanitizer.bypassSecurityTrustUrl(key);
            this.imgsrcs.push(this.sanitizer.bypassSecurityTrustUrl(key));

            this.all.push({ p: this.attractionList[i], imgsrc: this.attractionList[i].imgsrcs[key] })
