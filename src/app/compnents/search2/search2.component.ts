...
export class Search2Component implements OnInit {
  ...
  openDialog(imi): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      console.log(imi);
      var aa = this.itinerary.todo[imi.Dayno].AttractionPoints.find(
        (a) => a.AttractionName == this.dataget[0].AttractionName
      );
      if (!aa) {
  ...
  ngOnInit() {
    ...
    let Itinerary: Array<Itinerary> = JSON.parse(
  ...
  changestay(imi) {
    ...
    if (this.number == 0) {
      ...
      console.log(ind1);
      if (ind1 > 0) this.staypointsname.splice(ind1);
      console.log(this.staypointsname);
    } else {
  ...
  deletingattr(it, item) {
    ...
    var ass = this.itinerary.todo[as].AttractionPoints.findIndex(
      (a) => a.AttractionName == it
    );
    console.log(ass);
  ...
  changestaypoint(im, imi) {
    ...
    this.itinerary.todo[ind].AttractionPoints = this.attractionList.filter(
      (a) => a.City == this.itinerary.todo[ind].cityname
    );
    if (this.itinerary.todo[ind].AttractionPoints.length > 1) {
      this.itinerary.todo[ind].AttractionPoints.splice(
        this.itinerary.todo[ind].AttractionPoints.length - 1
      );
    }
    ...
    for (var i = 0; i < this.itinerary.todo[ind].CurrentRoute.length; i++) {
      ...
      if (
        this.flag == 1 &&
        this.itinerary.todo[ind].CurrentRoute[i].EndPoint != im
      ) {
    ...
    if (this.itinerary.todo[ind + 1]) {
      ...
      for (
        var i = 0;
        i < this.itinerary.todo[ind + 1].CurrentRoute.length;
        i++
      ) {
